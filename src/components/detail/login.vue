    ...
    <div class="index">
      <div class="index-main">
        <div class="kxk-box flex fw">
          ...
          <div class="login-btn flex cur" @click="_login">登录</div>
          <router-link tag='div' to='/login/register' class="login-btn flex cur" replace>
            没有账号？去注册
          </router-link>
...
<script>
  ...
  const QQ_URL = 'https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id=101542054&redirect_uri=http%3a%2f%2fwww.kexuekao.com%2f%23%2ftxLogin&state=text';
  import {send_message, code_login} from 'api/index'
  export default {
    name: "login",
    data() {
      return {
    ...
    methods: {
      ...
      async _login() {
        ...
        const ret = await code_login(this.phone, this.code);
